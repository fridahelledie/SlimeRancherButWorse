...
public class Player : MonoBehaviour
{
    ...
    [Tooltip("Time taken, in seconds, to reach maximum speed from a standstill.")]
    ...
    void Movement()
    {
        //every frame reset local movement direction to zero and set X and Z based on WASD
    ...
    void VelocityLoss()
    {
        //Lose velocity as lang as we are grounded, and we are either not holding any movement keys or faster than movespeed
        if (grounded && (localMovementDirection == Vector3.zero || worldVelocity.magnitude > movespeed))
        {
            ...
            //Otherwise if we're losing less velocity:
                ...
                //Apply velocity loss in the opposite direction of the worldvelocity:
    ...
    void Gravity()
    {
        ...
        {
            //Decrese Y velocity by gravityPerSecond, but dont go under -maxGravity
    ...
    void ApplyVelocity()
    {
        //While grounded, apply slight downward velocity to keep grounded state correct
        if (grounded)
        {
            yVelocity = -1;
        }

        //calculate the movement recieved this frame
        Vector3 movementThisFrame = (worldVelocity + (Vector3.up * yVelocity)) * Time.deltaTime;

        //Calculate where we expect to be after moving if we do not hit anythin
        Vector3 predictedPosition = trans.position + movementThisFrame;

        //Only call Move, if we have a minimum of 0.03 velocity
        if (movementThisFrame.magnitude > 0.03f)
        {
