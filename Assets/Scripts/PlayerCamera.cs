...
public class PlayerCamera : MonoBehaviour
{
    ...
    [Tooltip("The amount of smoothing applied to the third-person camera.A higher value will cause the camera to more gradually turnwhen the mouse is moved.")]
    [Range(0, .99f)]
    ...
    //Bounds:
    [Header("Bounds")]
    [Tooltip("Minimum distance from the for the third person camera to havefrom its orbit point.")]
    ...
    [Tooltip("Multiplier for scroll wheel movement. A higher value will result in a greater change in third-person distance when scrolling themouse wheel.")]
    ...
    [Tooltip("Is the camera currently in first-person mode (true) or thirdperson mode (false)? This can be set to determine the default mode when the game starts.")]
    ...
    private Vector3 TargetForwardDirection
    {
        get
        {
            //return the forward axis og the TargetRotation
    ...
    // The LateUpdate event in Unity functions similarly to Update, as it is called every frame,
    // but it runs after all Update methods across all scripts have executed.
    // This is useful when you want to ensure that certain operations, like camera updates,
    // occur after other updates, such as player movements.
    private void LateUpdate()
    {
        //update camera taget rotation, so long as we are not showing the mouse cursor
        if (!showingMouseCursor)
            UpdateTargetRotation();

        //perform positioning logic based on the mode we are in
         if (firstPerson)
            FirstPerson();
         else
            ThirdPerson();
    ...
    void HotKeys()
    {
        ...
        if (Input.GetKeyDown(mouseCursorShowHotkey))
        {
            ...
            SetMouseShowing(true); //show mouse
        }

        if (Input.GetKeyUp(mouseCursorShowHotkey))
        {
            //whenever the mouse cusor hotkey is let go of
            SetMouseShowing(false); //do not show the mouse
        }

        //scroll wheel for third-person distance
        if (!firstPerson) //only check for it while we are in third mode
        {
            //get scroll wheel delta this frame
            float scrollDelta = Input.GetAxis("Mouse ScrollWheel");

            //subtract delta from third person distance, multiplying it by the scroll sensitivity
    ...
    {
        //The x rotation we should be receving uses the mouse Y because rotating along the X axis makes us look up/down
    ...
    void FirstPerson()
    {
        //if the camera is not in its first person position, move it there
        Vector3 targetWorldPosition = modelHolder.TransformPoint(firstPersonLocalPosition);

        if(trans.position != targetWorldPosition) 
            //if the camera is not at the first person camera yet
            {
            //lerp the camera towards the first person camera location
            trans.position = Vector3.Lerp(trans.position, targetWorldPosition, 2.0f);
    ...
    void ThirdPerson()
    {
        ...
        else //if the ray did not hit anything
        {
